...
{
    const Vec2i chunk_pos = {
        {cell_pos->x / CHUNK_DIM, cell_pos->y / CHUNK_DIM}
    };
...
{
    Cell *cell = world_get_cell(cell_pos);
    cell->id = ID_EMPTY;
    cell->tempreture = 0.0f;
}
...
inline void world_set_cell(const Vec2i *cell_pos, const Cell *cell)
{
    const Vec2i cell_pos_in_chunk = {
        {cell_pos->x % CHUNK_DIM, cell_pos->y % CHUNK_DIM}
    };
...
void world_update()
{
	...
	for (i32 chunk_y = 0; chunk_y < world.height; chunk_y++) {
		for (i32 chunk_x = 0; chunk_x < world.width; chunk_x++) {
            Vec2i chunk_pos = {{chunk_x, chunk_y}};
